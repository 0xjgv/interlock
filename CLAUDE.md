...
- Acceptance: `interlocks acceptance` — Gherkin scenarios via pytest-bdd (default, shares coverage with `test`); also writes `.interlocks/trace.json` (scenario → public-symbol map) via the bundled pytest plugin. Falls back to behave when `features/steps/` + `features/environment.py` are present or `acceptance_runner = "behave"` (trace plugin skipped). No-ops silently when no `features/` directory exists. `run_acceptance_in_check = true` opts the `check` stage into running it. Blocking in `ci`.
- Acceptance baseline: `interlocks acceptance baseline [--force]` — seeds `.interlocks/acceptance_budget.json` (signed snapshot of `untraced_public_symbols`); refuses to overwrite without `--force`; refuses to run when the trace map is stale.
- Acceptance status: `interlocks acceptance status` — read-only summary (scenario count, traced/untraced symbol count, delta vs. budget); names the behave gap.
- Trace artifact: `.interlocks/trace.json` is committed to the repo for reproducibility. `interlocks ci` enforces a monotonic budget on the *set* of untraced public symbols once `.interlocks/acceptance_budget.json` exists; tampering (hand-edits that grow `untraced`) fails with `budget tampering detected`.
...
acceptance_trace_path = ".interlocks/trace.json"           # scenario→symbol map (commit it)
acceptance_budget_path = ".interlocks/acceptance_budget.json" # signed untraced-symbol snapshot
enforce_acceptance_budget = true # auto-detect on file presence; explicit override allowed
...
### Acceptance trace + budget commit policy

`.interlocks/` is `.gitignore`'d wholesale, with two explicit unignores:

- `.interlocks/trace.json` — scenario → symbol map produced by `interlocks acceptance`. Committed for reproducibility (small, deterministic, regenerated by every acceptance run). If the suite grows large enough that diffs become noisy, a future change can move it back to `.gitignore` and rely on per-PR regeneration.
- `.interlocks/acceptance_budget.json` — signed snapshot of `untraced_public_symbols`. Committed because `interlocks ci` enforces a monotonic ratchet against it; without the file in-repo, every clone would have to re-baseline.

Everything else under `.interlocks/` (e.g. `ci.json` evidence, `trust.json` cache) stays gitignored.
