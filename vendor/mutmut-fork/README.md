...
as the distribution name **`pyharness-mutmut`** so that pyharness can be
installed cleanly via `pipx install pyharness` (or `uv tool install pyharness`)
without requiring `git` on the install host.

## Why this exists

Pyharness currently depends on a specific upstream SHA that includes the
`set_start_method` guard (see [mutmut#466](https://github.com/boxed/mutmut/pull/466)),
which has not yet been released on PyPI. To avoid forcing every pyharness
...
that includes the fixes pyharness depends on, this fork will be retired and
pyharness will switch back to the upstream distribution.
