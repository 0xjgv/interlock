...
- Stable `interlock <task>` CLI surface with four one-command stages:
...
- Fake-confidence detectors: blocking CRAP gate (`enforce_crap`) and
  mutation testing via mutmut (`interlock mutation`, wired into `interlock nightly`
...
- Acceptance tests via pytest-bdd (default) or behave (auto-detected), with
  `interlock init-acceptance` scaffold and self-dogfooded Gherkin coverage of
  the CLI surface.
- Auxiliary tasks: `interlock audit` (pip-audit), `interlock deps` (deptry),
  `interlock arch` (import-linter), `interlock coverage`, `interlock version`.
...
- The blocking CRAP gate is on by default (`enforce_crap = true`).
  Set `enforce_crap = false` under `[tool.interlock]` to fall back to
  advisory mode (print offenders, exit 0).
- `interlock mutation` stays advisory by default — mutmut runtime is
  unbounded on real codebases. Gate it per-PR with
  `run_mutation_in_ci = true` + `enforce_mutation = true`, or
  schedule `interlock nightly` for a bounded, blocking run.
