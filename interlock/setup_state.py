...
``interlock.stages.setup_hooks`` (the writer) and ``interlock.tasks.doctor`` (the reader),
...
def is_post_edit_command(command: object) -> bool:
    ...
    return isinstance(command, str) and (
        command.endswith("interlock.cli post-edit") or command == "uv run interlock post-edit"
    )
...
def pre_commit_hook_installed(project_root: Path) -> bool:
    ...
    try:
        ...
        return False
    return "interlock.cli pre-commit" in body or "interlock pre-commit" in body
