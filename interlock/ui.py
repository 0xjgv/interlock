...
from interlock import __version__
...
def banner(cfg: InterlockConfig) -> None:
    """One-line stage banner: `interlock vX  ·  preset=Y  ·  runner=Z  ·  invoker=W`."""
    if is_quiet():
        return
    preset = cfg.preset or "none"
    parts = [
        f"interlock v{__version__}",
        f"preset={preset}",
...
def command_banner(command: str, cfg: InterlockConfig | None = None) -> None:
    ...
    if is_quiet():
        return
    parts = [f"interlock v{__version__}", f"command={command}"]
    if cfg is not None:
        parts.extend([
