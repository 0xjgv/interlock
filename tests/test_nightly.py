...
def test_nightly_injects_min_score_from_config(
    ...
    assert sys.argv == ["interlocks", "nightly", f"--min-score={load_config().mutation_min_score}"]
...
def test_nightly_preserves_user_supplied_min_score(
    ...
    monkeypatch.setattr(sys, "argv", ["interlocks", "nightly", "--min-score=42.5"])

    nightly_mod.cmd_nightly()

    assert sys.argv == ["interlocks", "nightly", "--min-score=42.5"]
