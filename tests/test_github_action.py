...
from harness import github_action


def test_command_from_args_defaults_to_harness_ci() -> None:
    assert github_action._command_from_args(()) == ["harness", "ci"]


def test_command_from_args_accepts_command_override() -> None:
    assert github_action._command_from_args(("--command", "harness ci --verbose")) == [
        "harness",
        "ci",
        "--verbose",
    ]
...
def test_write_summary_records_command_and_success(
    ...
    github_action.write_summary(["harness", "ci"], 0)
...
def test_write_summary_records_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    github_action.write_summary(["harness", "ci"], 2)
...
def test_main_exits_with_command_status(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    with pytest.raises(SystemExit) as exc:
        github_action.main(["--command", "harness ci"])

    assert exc.value.code == 7
    assert calls == [["harness", "ci"]]
...
def test_action_metadata_delegates_to_harness_ci() -> None:
    ...
    assert "default: harness ci" in action
    assert 'python -m harness.github_action --command "${{ inputs.command }}"' in action
