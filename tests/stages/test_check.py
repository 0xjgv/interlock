...
def test_check_fixes_trivially_fixable_lint(tmp_project: Path) -> None:
    ...
    (tmp_project / "tmppkg" / "core.py").write_text(dirty, encoding="utf-8")

    result = _run_check(tmp_project)

    assert result.returncode == 0, f"stdout={result.stdout}\nstderr={result.stderr}"
    assert "import os" not in (tmp_project / "tmppkg" / "core.py").read_text(encoding="utf-8")
