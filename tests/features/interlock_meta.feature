Feature: Interlock meta commands
  As a user adopting interlock in a fresh project
  ...
  Scenario: Acceptance is a silent no-op when features/ is missing
    ...
    When I run "interlock acceptance" in the tmp project
  ...
  Scenario: init-acceptance scaffolds the canonical layout and refuses to overwrite
    ...
    When I run "interlock init-acceptance" in the tmp project
    Then the command exits successfully
    ...
    When I run "interlock init-acceptance" in the tmp project a second time
    Then the command exits with a non-zero status

  Scenario: setup-hooks installs an executable pre-commit hook
    Given a tmp project with a .git directory
    When I run "interlock setup-hooks" in the tmp project
    Then the command exits successfully
