"""GitHub Action helper for running pyharness CI and writing job summaries."""
...
DEFAULT_COMMAND = "harness ci"


def main(argv: Sequence[str] | None = None) -> None:
    """Run the configured pyharness command and exit with its return code."""
...
def _command_from_args(args: Sequence[str]) -> list[str]:
    ...
    if args[0] != "--command":
        raise SystemExit("usage: python -m harness.github_action [--command 'harness ci']")
    if len(args) != 2:
